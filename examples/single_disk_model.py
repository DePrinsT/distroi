...
if __name__ == '__main__':
    import sys
    sys.path.append('..')  # Add project root to python path so this script can be run from the terminal.
    ...
    img = image_fft.read_image_fft_mcfost(img_path=f'{mod_dir}{img_dir}/RT.fits.gz', disk_only=False)
    img.diagnostic_plot(f'{fig_dir}/fft', log_plotv=False, show_plots=True)
    ...
    # img.diagnostic_plot(fig_dir, plot_vistype='fcorr', log_plotv=True, show_plots=True)
    # print(img.freq_info())
    #
    # # Monochromatic model observables test
    # img_dir = 'MATISSE_N/data_10.0/'
    # container_data = oi_observables.read_oicontainer_oifits(data_dir, data_file, wave_lims=(9.75, 10.20), fcorr=True)
    # container_model = oi_observables.calc_mod_observables(container_data, mod_dir, img_dir, monochr=True,
    #                                                         ebminv=1.4, read_method='mcfost')
    ...
    # img_fft_list = image_fft.get_image_fft_list(mod_dir, img_dir)
    # for img_fft in img_fft_list:
    #     img_fft.redden(ebminv=1.4)
    # container_model = oi_observables.calc_mod_observables(container_data, img_fft_list)
    # oi_observables.plot_data_vs_model(container_data, container_model, fig_dir=fig_dir, log_plotv=True,
    #                                   show_plots=True, plot_vistype='vis')
